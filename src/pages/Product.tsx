  ...
  if (!product) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center pt-20">
        ...
        <button onClick={() => navigate("/shop")} className="text-gray-500 hover:text-black uppercase text-sm tracking-widest border-b border-black">
          Back to Shop
        </button>
      </div>
    );
  ...
  return (
    <div className="pt-24 pb-24 px-4 md:px-8 max-w-7xl mx-auto min-h-screen">
      ...
      <div className="flex items-center gap-2 text-xs font-semibold text-gray-400 uppercase tracking-widest mb-8">
        ...
        <button onClick={() => navigate(`/shop?category=${product.category}`)} className="hover:text-black transition-colors">{product.category}</button>
        <ChevronRight size={14} />
        <span className="text-black overflow-hidden text-ellipsis whitespace-nowrap">{product.name}</span>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-12 lg:gap-24 mb-24">
        ...
        <div className="w-full lg:w-1/3 flex flex-col justify-center">
          <h1 className="text-4xl md:text-5xl font-heading font-black uppercase tracking-tighter mb-4">{product.name}</h1>
          <div className="flex items-center gap-4 mb-8 text-[#141414]">
            <span className="text-2xl font-bold">{formatNPR(product.price)}</span>
            {product.oldPrice && (
              <span className="opacity-30 line-through text-lg font-medium">{formatNPR(product.oldPrice)}</span>
            )}
            <div className="flex gap-2">
              {product.isNew && (
                <span className="bg-white border border-[#141414]/10 text-[#141414] px-2 py-1 text-[9px] font-bold uppercase tracking-tighter">New</span>
              )}
              {product.isSale && (
                <span className="bg-black text-white px-2 py-1 text-[9px] font-bold uppercase tracking-tighter">Sale</span>
              )}
              {product.isBestSeller && (
                <span className="bg-[#141414] text-white px-2 py-1 text-[9px] font-bold uppercase tracking-tighter">Best Seller</span>
              )}
            </div>
          </div>
          ...
          <div className="mb-8">
            <div className="flex justify-between items-center mb-4">
              <span className="font-bold uppercase tracking-widest text-[11px] text-[#141414]/40">Size</span>
              <button className="text-[11px] font-bold text-[#141414]/40 border-b border-[#141414]/40 hover:text-[#141414] hover:border-[#141414] uppercase tracking-widest transition-colors">Size Guide</button>
            </div>
          ...
          <div className="mb-8 space-y-6">
            ...
            <div>
              <label className="block font-bold uppercase tracking-widest text-[11px] text-[#141414]/40 mb-4">Special Instructions (Optional)</label>
              <textarea 
                value={instructions}
          ...
          <div className="space-y-4">
            <div className="flex gap-4 w-full">
              ...
              >
                <Heart size={20} className={isWishlisted ? "fill-black text-black" : "text-[#141414]"} />
              </button>
            </div>
            <a 
              href={whatsappLink}
      ...
      <div className="pt-20 mt-24 border-t border-black/5">
        <div className="flex justify-between items-end mb-8">
          <h2 className="text-[11px] font-bold uppercase tracking-widest text-[#141414]/40">You May Also Like</h2>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8">
          {featured.filter(p => p.id !== product.id).slice(0, 4).map(related => (
